...
@app.route("/boq-projects/<int:pid>/buildings/<int:bid>/floors/<int:fid>/complete/save-all", methods=["POST"])
@login_required
def boq_floor_complete_save_all(pid, bid, fid):
    """Save the WHOLE Complete BOQ form in one shot.

    The Complete BOQ page wraps every section's editable rows in ONE
    form. This handler iterates every ticked row, deriving its target
    section from the parallel bill_no[] / section_letter[] arrays, and
    INSERTs into boq_floor_items + boq_floor_rate_buildup. Floor-wide
    rates (overhead, profit, vat, vat_in_basic) apply to every row.

    Idempotent in the sense that re-running with the SAME ticked rows
    creates duplicate items -- the user expects this to be a "fresh
    save" of the floor, not a top-up. To prevent dupes the page can
    untick rows already saved (the existing rows are shown read-only
    above the editable rows).
    """
    uid = session["user_id"]
    project = _boq_project_owned_or_404(pid, uid)
    ...
    saved = 0
    skipped = 0
    # Track per-section next item number to avoid sequential gaps.
    next_no_by_sec = {}
    ...
    with get_db() as c:
        for i in range(n):
            ...
            if boq_rate_v3:
                ...
                supply_amt = basic * (row_sp + eff_vat) / 100.0
                install_amt = basic * (row_ip + floor_oh + floor_prf) / 100.0
    ...
    try:
        ...
        boq_audit(get_db, uid, "boq_complete_save_all", "boq_floor", fid,
