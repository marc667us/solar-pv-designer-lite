...
def register_capital_investment(app, *, get_db, login_required, csrf_protect,
    ...
    def _step9(pid: int):
        ...
        if request.method == "POST":
            ...
            with get_db() as c:
                # NOTE: we intentionally leave services_csv blank - the
                # BOQ standalone UI will let the user pick engineering
                # services on the standard flow if desired.
                try:
                    cur = c.execute(
                    ...
                    # Older schema without services_csv
                    cur = c.execute(
                        "INSERT INTO boq_projects "
                        "(user_id, project_name, client_name, location, "
                        " project_type, external_works_included, "
                ...
                # 2. Create a boq_buildings row per enabled facility.
                for b in selected_buildings:
                    label = next(
                    ...
                    try:
                        c.execute(
                            ...
                            " primary_purpose, purpose_subtype, "
                            " building_area, number_of_floors, "
                            " basement_included, roof_level_included, "
                            " external_area_included) "
                            "VALUES (?,?,?,?,?,?,?,?,?,?)",
                            (new_boq_pid, label, b.upper(),
            ...
            _save_project_field(pid, "boq_project_id", str(new_boq_pid))
            flash(
                f"Linked BOQ project #{new_boq_pid} created with "
                f"{len(selected_buildings)} building(s). Configure line "
                f"items in the standard BOQ engine.",
                "success",
            )
            return redirect(url_for("capital_investment_project", pid=pid))
