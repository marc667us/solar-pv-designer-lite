...
# ---------------------------------------------------------------
# Generic asset download - serves the build script output PDFs
# and MP3 walkthroughs straight from docs/. Whitelisted slug map
# prevents path traversal. Added 2026-06-17 so the support page
# can surface the freshly-built tutorial collateral without a
# bespoke route per file.
# ---------------------------------------------------------------
_SUPPORT_ASSETS = {
    # PDF tutorials (regenerated by build_collateral_pdfs.py)
    "user-guide":       ("SolarPro_User_Guide.pdf",       "application/pdf"),
...
@login_required
def support_asset(slug):
    """Serve a tutorial asset from the docs/ directory.

    `slug` MUST appear in _SUPPORT_ASSETS - any other value 404s,
    so the route is safe against path traversal (no slash in the
    slug ever reaches the filesystem).
    """
    entry = _SUPPORT_ASSETS.get(slug)
    if not entry:
        abort(404)
    filename, mime = entry
    ...
    if not os.path.isfile(path):
        abort(404)
    # as_attachment=False so the browser opens the PDF inline; the
    # template already exposes a separate "Download" link if needed.
...
def project_email(pid):
    ...
    REPORT_OPTIONS = [
        ...
        ("Shading Analysis Report",     url_for("export_pdf_shading",     pid=pid)),
...
## 5b. Concluding Action - Sizing Decision

"""
    _orig_panels = int(r.get("num_panels", 0) or 0)
    _panel_wp = int(r.get("panel_wp", 400) or 400)
    _extra_kwp = pv_corrected - pv_base
    _extra_panels = int(math.ceil(_extra_kwp * 1000 / _panel_wp)) if _panel_wp > 0 else 0
    _new_panels = _orig_panels + _extra_panels
    _daily_kwh = float(r.get("daily_kwh", 0) or 0)
    if factor >= 0.999:
        md += (f"No shading was detected on this site. The original PV array of "
               f"**{_orig_panels} panels ({pv_base:.2f} kWp)** will deliver the "
               f"design daily energy demand of **{_daily_kwh:.2f} kWh/day** without "
               f"adjustment. **No re-sizing required.**\n\n")
    else:
        _useful = pv_base * factor
        md += (f"Due to **{loss_pct:.1f}% shading loss** ({label}) at this site, "
               f"the original PV array of **{_orig_panels} panels "
               f"({pv_base:.2f} kWp)** would only deliver about "
               f"**{_useful:.2f} kWp** of usable capacity against the design "
               f"daily energy demand of **{_daily_kwh:.2f} kWh/day**.\n\n"
               f"**Therefore**, the array must be increased to "
               f"**{_new_panels} panels ({pv_recommended} kWp installed)** "
               f"so the shaded array still produces the required energy. "
               f"That is **{_extra_panels} additional panels** "
               f"({_extra_kwp:.2f} kWp extra) beyond the un-shaded design.\n\n")
        if factor < 0.85:
            _mit = "DC optimisers" if factor >= 0.75 else "micro-inverters (Enphase IQ8)"
            md += (f"**Alternative path:** install {_mit} to recover most of the "
                   f"loss without adding panels - capex vs roof-area trade-off.\n\n")
    md += """---
...
def admin_marketplace_pending():
    ...
    with get_db() as c:
        ...
        if cat_id:
            ...
            args.append(cat_id)
        if q:
            # LOWER() on both sides for cross-dialect case-insensitive search
            # (matches the same pattern used on the public /marketplace).
            sql += ("AND (LOWER(ec.name) LIKE ? OR LOWER(ec.brand) LIKE ? "
...
def rfqs_view(rfq_id):
    ...
    with get_db() as c:
        ...
        # Suggested-supplier set: any supplier behind a product the
        # buyer already added to this RFQ. Pre-ticks the checkboxes
        # so the buyer doesn't have to hunt for them in the list.
        _pids = [int(it["product_id"]) for it in items if it["product_id"]]
        suggested_supplier_ids = set()
        if _pids:
            _ph = ",".join(["?"] * len(_pids))
            for _r in c.execute(
                f"SELECT DISTINCT supplier_id FROM equipment_catalog WHERE id IN ({_ph})",
                _pids,
            ).fetchall():
                if _r["supplier_id"]:
                    suggested_supplier_ids.add(int(_r["supplier_id"]))
    return render_template(
        "rfq_view.html",
        user=current_user(),
        rfq=rfq,
